...
A side task that reuses the Telegram channel: find the best used-car deals and text a ranked top-N. Helper: `car_search.py`. Buyer wants a RAV4-Hybrid-like car: **silver or LIGHT gray exterior only (NO dark/blue-leaning grays — graphite, magnetic, gunmetal, etc.), a leather interior in any color EXCEPT a solid black cabin (warm tan/beige/cream, neutral gray, and two/three-tone all pass — interior restriction RELAXED 2026-06-10 after the strict warm-only filter returned zero verified cars near LA), hybrid, 2022+, under 50k mi, $20k–$40k.** Location: greater LA for franchise/used dealers, but **anywhere via Carvana / CarMax** (they deliver) when the buyer asks to widen.

**Buyer's current front-runner (2026-06-10):** the **Mitsubishi Outlander PHEV SEL** — buyer "really likes it" after it topped the hardened shortlist (the Bellflower car at $28,985, 27k mi, Graphite Gray, leather). Wants to comparison-shop *that specific model* used. Reliability context to set expectations: the 2022+ Outlander PHEV (on the Nissan Rogue/Alliance platform) earns roughly **average** predicted-reliability scores (J.D. Power ~80/100; Consumer Reports middling) — not Toyota/Lexus-tier, but the PHEV drivetrain has been comparatively trouble-free and it carries Mitsubishi's long **10yr/100k-mi powertrain warranty** (transferable portion 5yr/60k), a real plus on a used buy. Watch-outs: infotainment glitches and the usual first-gen-redesign squeaks; confirm the high-voltage battery health / remaining warranty on any used unit. **Repairability:** shares the Nissan Rogue platform + many parts, so routine parts are cheap/easy and the 2.4L is conventional to service — but PHEV/high-voltage work needs a Mitsubishi specialist and the dealer network is thinner than Toyota/Honda. **NVH:** reviewers call the cabin quiet/upscale for the class (acoustic windshield + active noise cancellation on SEL, near-silent in EV mode, road/wind noise well-damped); only gripe is a coarse 2.4L under hard throttle. Verified used SoCal SEL options were thin (2 dealer-confirmed: STG Bellflower 2024 $28,985 / Camacho Palmdale 2025 $32,600); buyer reported the Cerritos car sold despite the dealer feed still listing it.
...
3. **Filter** —
   ...
   - **Exterior: silver / LIGHT gray ONLY — white is excluded.** `car_search.is_silver` uses a silver/gray allowlist (silver, gray/grey, chrome, platinum, titanium, stardust, sterling) PLUS a dark denylist (graphite, magnetic, gunmetal, charcoal, granite, cement, meteorite, machine gray, polymetal, urban gray, mercury) that **vetoes** a match. White, pearl, glacier, frost, and other white-adjacent words were removed 2026-06-11 per buyer preference — silver/gray only.
   - **Interior: leather, any color but solid black.** Warm tones (tan/beige/cream/saddle), neutral grays, and two/three-tone combos all pass; only an all-black cabin is vetoed (`car_search.has_acceptable_interior`). RELAXED 2026-06-10 from the original warm-only rule, which returned zero verified near-LA cars. Search-results pages rarely show interior color, so read it off the detail page / dealer site, and treat interior the same trim-spec way as leather (confirm, don't guess).
...
5. **Rank by quality** — year (dominant, newer = better), then mileage. **Price is NOT a ranking factor** — the buyer already set a $20k–$40k range, so within-range price differences don't signal quality. RAV4 Hybrid gets a priority score boost (~1.5 year equivalent) since it's the buyer's primary target. Best quality = #1.
...
Availability-hardening run (2026-06-10): buyer reported the #1 car (2023 Outlander PHEV SEL, Cerritos, $25,975, VIN JA4T5VA91PZ058501) was NOT actually in stock — yet Cars.com showed it live in both search results and on the detail page ("Check availability" active). Root cause: **Cars.com lags the dealer feed and keeps sold cars live; it is not authoritative.** Re-verified the whole list by VIN against each dealer's OWN site: 2 were already SOLD (Cerritos Outlander; Santa Ana Tucson N Line at California Motors Direct), and 2 (CarMax Murrieta Tucson, CarMax Burbank Wrangler) were unverifiable because CarMax 403-blocks bots. Hardened the workflow + `car_search.py`: VIN-on-dealer-site is the only proof, added `check_listing_available()`, `UNVERIFIABLE_SELLERS`/CarMax drop rule, and a `send_ranked()` guard that refuses listings lacking `vin`+`verified`. Sent a hardened, VIN-confirmed shortlist of 7 (Mitsubishi, Toyota×2, Lexus, Mazda, Hyundai, Honda; $28,985–$38,788). **Follow-up:** the strict bar + CarMax 403s + Kia exclusion make a full 10-car / 7-brand list hard to fill with auto-verifiable cars.
