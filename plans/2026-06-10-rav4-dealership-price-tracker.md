...
3. **Filter** —
   ...
   - **Exterior: silver / LIGHT gray ONLY.** `car_search.is_silver` now uses a light allowlist (silver, ice/alloy/shimmering/billet silver, sterling, glacier, light gray, platinum) PLUS a dark denylist (graphite, magnetic, gunmetal, steel, charcoal, granite, cement, titanium, meteorite, machine gray, polymetal, urban gray, mercury) that **vetoes** a match. The buyer flagged a "Graphite Gray" Outlander as looking blue — when in doubt, exclude. Drop black/white/blue/red and all dark grays.
