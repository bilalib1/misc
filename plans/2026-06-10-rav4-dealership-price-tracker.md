...
A side task that reuses the Telegram channel: find the best used-car deals and text a ranked top-N. Helper: `car_search.py`. Buyer wants a RAV4-Hybrid-like car: **silver or LIGHT gray exterior only (NO dark/blue-leaning grays — graphite, magnetic, gunmetal, etc.), a LIGHT/warm leather interior (tan, brown, beige, off-white, eggshell, cream), hybrid, 2022+, under 50k mi, $20k–$40k.** Location: greater LA for franchise/used dealers, but **anywhere via Carvana / CarMax** (they deliver) when the buyer asks to widen.
...
3. **Filter** —
   ...
   - **Interior: LIGHT / warm leather.** Must be tan, brown, beige, off-white, eggshell, cream, saddle, etc. (`car_search.has_light_interior`) — not a black cabin. Search-results pages rarely show interior color, so read it off the detail page / dealer site, and treat interior the same trim-spec way as leather (confirm, don't guess).
