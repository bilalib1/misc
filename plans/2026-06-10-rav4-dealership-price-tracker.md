...
4. **Verify finalists — by VIN, on the SELLER'S authoritative page (hardened).** Cars.com is NOT proof of availability: it keeps sold cars live in BOTH search results and the detail page (we once texted a sold Outlander because its Cars.com detail page still showed "$25,975 / Check availability"). The only acceptable confirmations:
   ...
   - **Carvana / online-only sellers:** the seller's live page is authoritative (a sold car's page disappears), so a current carvana.com listing is good evidence — but it can't usually be auto-checked (see below).
   - **Unverifiable sellers → drop.** CarMax and Carvana hard-block automated checks (HTTP 403, Akamai). If you can't confirm a VIN, treat it as NOT in stock and leave it out (or hand the buyer the link to tap-confirm). `car_search.check_listing_available()` is a best-effort probe; it returns `unknown` on these 403s by design.
